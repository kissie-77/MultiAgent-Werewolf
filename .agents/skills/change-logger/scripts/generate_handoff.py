...
Pulls mechanical info from docs/changes/<slug>.md, git status, and git log,
then writes placeholder sections (`<!-- FILL: ... -->`) for Codex or a human
to fill semantically.
...
def main() -> int:
    ...
    if changed_files:
        ...
        parts.append("_(no entries in change log - either the task just started or the hook did not fire)_")
    parts.append("")
    ...
    parts.append("")
    parts.append("<!-- FILL: BLOCKERS - what is stuck, what you tried that did not work, deliberately deferred items. Empty if none. -->")
    parts.append("")
    parts.append("## Next steps")
    parts.append("")
    parts.append("<!-- FILL: NEXT_STEPS - ordered list. Pretend you are handing this to a competent stranger who has read the code but not the conversation. -->")
    parts.append("")
    parts.append("---")
    parts.append("")
    parts.append("_Skeleton from `.agents/skills/change-logger/scripts/generate_handoff.py`. Sections marked `<!-- FILL: ... -->` must be filled before this handoff is useful to the next AI._")
    parts.append("")
