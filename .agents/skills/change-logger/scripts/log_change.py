...
Reads a Codex hook payload from stdin and appends a row to
docs/changes/<current-task>.md describing the edit. Silent on success;
failures are written to .codex/change-logger.err and the script still
...
def project_root(cwd: str) -> Path:
    ...
    for cand in [p, *p.parents]:
        if (cand / ".codex").exists() or (cand / ".agents").exists() or (cand / ".git").exists():
            return cand
    return p
...
def log_err(root: Path, msg: str) -> None:
    try:
        ...
        with (root / ".codex" / "change-logger.err").open("a", encoding="utf-8") as f:
...
def main() -> int:
    ...
    if (
        ...
        or rel.startswith(".agents/skills/change-logger/")
        or rel.startswith("docs/changes/")
        or rel == "handoff.md"
    ):
        return 0
...
if __name__ == "__main__":
    try:
        ...
        log_err(project_root(os.getcwd()), f"unexpected failure: {exc}")
        sys.exit(0)
