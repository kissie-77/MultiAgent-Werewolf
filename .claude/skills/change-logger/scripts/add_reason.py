...
def main() -> int:
    ...
    ap.add_argument("--task", default=None,
                    help="Override task slug (default: read from .claude/current-task)")
    args = ap.parse_args()
    ...
    if not slug:
        ...
        if not task_file.exists():
            print("no .claude/current-task — run start_task.py first or pass --task", file=sys.stderr)
            return 1
    ...
    for i in range(len(lines) - 1, -1, -1):
        ...
        if "_PENDING_" in line:
            ...
            continue
        if args.count:
            # in --count mode, keep scanning upward through filled rows
            continue
        # default mode: stop when we encounter a filled data row
