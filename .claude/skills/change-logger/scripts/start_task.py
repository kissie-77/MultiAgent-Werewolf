...
Writes the slug to .claude/current-task and creates docs/changes/<slug>.md
...
def main() -> int:
    ...
    (claude_dir / "current-task").write_text(slug + "\n", encoding="utf-8")
    ...
    if not log_file.exists():
        header = (
            ...
            f"> Task name: {raw}\n"
            f"> Started: {now}\n"
            f"> Tracked via `.claude/skills/change-logger`\n\n"
            f"## Session 1 — {now}\n\n"
