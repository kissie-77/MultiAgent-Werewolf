...
私密 LLM 决策前 → InformationHub._refresh_actor_belief_skills
  → refresh_player_belief_skills → 按当前 BeliefState 匹配 Skill → 写入 _belief_skill_context
  → get_context_for_decision / context_builder 拼进 prompt
on_round_end  → 压缩工作记忆
on_game_end   → 更新 skill 权重 + 可选语义候选提炼
```

覆盖路径：圆桌发言、`collect_speech`、选座/投票、女巫夜间、yes/no、多目标、投票意向与 mind state 批量采集（经 `_merge_private_context` 或 `_collect_vote_intentions`）。
...
Bridge 层负责将 LLM 输出解析为结构化决策。启用 structured output 时走 Pydantic schema；否则 Bridge 仍支持 `[[n]]` 等 legacy 文本解析作为回退（见 `agent_base.md` 说明）。

| 决策类型 | 说明 | 结构化字段 / 回退 |
|----------|------|-------------------|
| SpeechDecision | 圆桌发言 | `public_speech`（≥15字）+ `private_thought` |
| SeatChoiceDecision | 选座（刀人/守人/查验/投票） | `seat` + `reason`；回退 `[[座位号]]` |
...
### 9.4 Bridge 结构化回退（2026-06-02）

部分私密决策在 structured invoke 失败时采用**显式保守默认**，避免随机改局：

| 路径 | 失败默认 |
|------|----------|
| `request_yes_no` | `False` |
| `request_vote_intention` | `seat=0`（观望） |
| `request_seat_choice`（关键路径） | 可禁用随机；正式投票优先 vote intention |

上述路径打 warning，经 §9.3 同一 handler 写入 `provider_events.jsonl`。
