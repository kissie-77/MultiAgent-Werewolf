...
| 变量 | 用途 |
|------|------|
| `ARK_API_KEY` / `ARK_EP` | 豆包（火山方舟）密钥与端点 ID；**标准对局默认** |
| `DEEPSEEK_API_KEY` 等 | 其他 7 家供应商，见 [PROVIDERS.md](./PROVIDERS.md) |
| `WEREWOLF_SETTINGS_TOKEN` | 可选；保护 `GET/POST /settings/api-keys` |
| `AGENT_SERIAL_DELAY_SECONDS` | AgentScope 串行调用间隔，减轻 429 |

**标准对局配置**（唯一正式板子）：`configs/standard-4p.yaml` … `standard-16p.yaml`（全员豆包）。旧配置已移入 `configs/archive/`。

密钥可通过网页「枢纽引擎」写入 `.env`（`POST /api/v1/settings/api-keys`），或手动 `cp .env.example .env` 后稀疏填写。

改 `.env` 后先验证：

```bash
uv run werewolf configs/standard-6p.yaml
uv run python scripts/test_ark_connectivity.py   # Doubao/ARK 连通性，期望 STATUS: OK
```
...
- 供应商 env 模板：[PROVIDERS.md](./PROVIDERS.md) · `provider_registry.py`
- 标准板子说明：`configs/README.md`
