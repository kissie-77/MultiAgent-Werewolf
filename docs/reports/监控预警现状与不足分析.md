...
MultiAgent-Werewolf **目前没有生产级监控预警体系**（无 Prometheus / Sentry / Slack / Webhook / 告警阈值引擎）。现有能力主要是：
...
对局失败、结构化决策丢失、PostGame 流水线失败等问题，**不会主动通知任何人**，只能事后翻 `events.jsonl`、`run_meta.json`、日志或 eval 产物。
...
| 端点 | 位置 | 行为 |
|------|------|------|
| `GET /health` | `interface/api/app.py` | 固定 `{"status": "ok"}`，不探测 LLM、磁盘、在途对局 |
| `GET /api/v1/games/{run_id}/status` | `interface/api/routes/actions.py` | 读内存会话或 `run_meta.json` |
...
| 问题 | 影响 | 现状 |
|------|------|------|
| 静默降级过多 | fallback 发言/决策仅 warning，无计数阈值 | bridge / structured_invoke |
...
| API 429 率 | eval_agent / agentscope 日志 | 连续 3 次告警 |
