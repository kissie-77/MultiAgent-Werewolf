...
> **状态**：Phase 1 + MVP 已实现（2026-05-26）；Phase 2：LLM Skill 提取、benefit 阈值筛 Skill 待做  
> **吕祎晗修改记录**：[吕祎晗-修改记录.md](./吕祎晗-修改记录.md)（队友速览）  
> **原则**：评测层 LLM **统一走 AgentScope**；不新增 `import openai` 旁路；**不改**运行时 v2 Prompt 写入逻辑（仍 JSON draft）
...
3. **日志可见性视图**：按需求生成多种「读日志」切片，支持 **当局者 POV**，节省 Token。
4. **打分系统**：完善意向改变 + 预留 **收益分（Benefit Score）**；**Phase 1 全量 Skill 提取**，Phase 2 再按高分筛选。
5. **团队对齐**：模块边界清晰、产物 schema 稳定、与《发现问题》已知项挂钩。
...
| 结构化输出         | `strategy` 新增 `ReplayAnalysisDecision`、`SkillExtractionDecision` 等 Pydantic 模型，经 AgentScope structured 或 JSON 解析 |
...
| `camp:{werewolf|villager}` | 阵营       | 公开事件 + 该阵营成员可见私密事件                         | 阵营战术分析（可选）         |
  ...
  "skills": [
    {
      ...
      "evidence": {
        ...
        "event_refs": ["round_1/night/wolf_discussion"],
        "scores": { "intention": 0, "benefit": null }
      },
...
运行时由 `agent_team/skill_loader.py` 读取对应身份目录，追加到系统 Prompt 的「对局经验 Skill 卡片」段落。
...
| 文档                                              | 用途                |
| ----------------------------------------------- | ----------------- |
| [工程架构重构计划.md](./工程架构重构计划.md)                    | 六模块职责             |
| [吕祎晗-发现问题.md](./吕祎晗-发现问题.md)                    | 已知 bug / 待决策      |
| [prompt_tuning.md](./prompt_tuning.md)          | Prompt 版本与人工复盘    |
| [OpenAI 接口与可删除性分析.md](./吕祎晗-OpenAI接口与可删除性分析.md) | 禁止 AsyncOpenAI 旁路 |
