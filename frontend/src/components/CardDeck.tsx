...
export default function CardDeck() {
  ...
  const handleShowArt = (e: React.MouseEvent, p: Player) => {
    ...
    if (triggerCast) {
      ...
      let skillName = p.role + "展示";
      let skillSub = "SHOW IDENTITY CARD";
      let effectType: "inspect" | "heal" | "poison" | "bite" | "shoot" | "vote" | "rally" = "rally";
      ...
      if (p.role === "预言家") {
        ...
        effectType = "shoot";
      } else if (WOLF_CAMP.includes(p.role)) {
        skillName = p.role + "袭击";
        skillSub = "WOLF ATTACK";
        effectType = "bite";
      } else if (p.role === "村民" || p.role === "平民") {
        skillName = "村民";
        skillSub = "VILLAGER";
        effectType = "vote";
      }
      ...
      triggerCast({
        casterId: p.id,
        casterName: p.isUser ? `你 (玩家 ${p.id} 号)` : p.name,
        role: p.role,
        skillName,
        skillSub,
        targetId: null,
