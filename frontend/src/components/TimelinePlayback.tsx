...
import { Play, Pause, SkipBack, SkipForward, Moon, Sword, ScanEye, Skull, HeartPulse, MessageSquare, Handshake } from "lucide-react";
...
export default function TimelinePlayback({
  ...
  pastEvents.forEach((ev) => {
    if (ev.result === "DEAD" && ev.targetId) {
      deadPlayers.add(normalizeSeatId(ev.targetId));
    }
  });
      ...
      <div className="col-span-1 border-r border-zinc-900 bg-[#0c0c0c] p-4 flex flex-col hidden lg:flex">
              ...
              >
                ...
                >
                  ...
                  {!isDead && (viewScope === "ALL" || p.id === viewScope) && (
                    <span className="text-[8px] font-mono leading-none">{p.role}</span>
                  )}
                  {isDead && (
        ...
        <div className="mt-auto pt-6 border-t border-zinc-900/50">
          <div className="flex justify-center gap-4 text-[10px] font-mono text-zinc-500">
            <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-blue-500/50 border border-blue-500" />好人</span>
            <span className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-red-500/50 border border-red-500" />狼人</span>
      ...
      <div className="col-span-1 lg:col-span-2 flex flex-col min-h-0 relative bg-zinc-950">
            ...
            return (
              ...
              >
                ...
                <div className={`flex-1 rounded p-3 border ${isActive ? "bg-zinc-900 border-zinc-700" : "bg-zinc-950 border-zinc-900"}`}>
                  {ev.type === "speech" && (
                    <>
                      <div className="text-[10px] font-sans text-zinc-500 mb-1 font-bold">{normalizeSeatId(ev.playerId)} 发言</div>
        ...
        <div className="h-14 bg-zinc-950 border-t border-zinc-900 flex items-center justify-between px-4 relative z-10">
          <div className="flex items-center gap-2">
            <button type="button" aria-label="回到开头" onClick={() => setCursor(0)} className="w-8 h-8 flex items-center justify-center text-zinc-500 hover:text-zinc-300 disabled:opacity-50" disabled={cursor === 0}>
            ...
            <button type="button" aria-label="下一事件" onClick={() => setCursor((c) => Math.min(timeline.length - 1, c + 1))} className="w-8 h-8 flex items-center justify-center text-zinc-500 hover:text-zinc-300 disabled:opacity-50" disabled={cursor === timeline.length - 1}>
          ...
          <div className="flex-1 px-4">
            <div
              className="w-full h-1 bg-zinc-900 rounded-full relative cursor-pointer"
              role="slider"
              aria-label="回放进度"
              aria-valuemin={0}
          ...
          <div className="flex items-center gap-2 text-xs font-mono">
            <button type="button" aria-label="切换播放速度" onClick={() => setSpeed((s) => (s === 1 ? 2 : s === 2 ? 4 : 1))} className="px-2 py-1 bg-zinc-900 rounded text-zinc-400 hover:text-zinc-200 border border-zinc-800">
              {speed}x
            </button>
          </div>
        </div>
      ...
      <div className="col-span-1 border-l border-zinc-900 bg-[#0a0a0a] p-4 flex flex-col hidden lg:flex">
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded p-3 mb-4">
          ...
          <div className="text-xl font-serif font-bold text-yellow-500">
            ...
            <span className="text-sm text-zinc-600 font-sans font-normal">/ {seatPlayers.length}</span>
          </div>
        </div>
        ...
        <div className="bg-zinc-950 border border-zinc-900 rounded p-3 mb-4 mt-auto">
          <div className="text-[10px] font-sans text-zinc-500 mb-2">对局概要</div>
