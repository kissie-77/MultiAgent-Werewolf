"""Single-track prompt builder: system + per-role identity (Chinese)."""
...
class PromptManager:
    """Builds AgentScope-compatible chat messages for werewolf agents."""

    @staticmethod
    def build_system_prompt(player_name: str, seat_number: int, plan: str = "自由发挥") -> str:
        """System prompt shared by all roles."""
    ...
    def build_identity_prompt(definition: RoleDefinition) -> str:
        """Identity prompt for a single role."""
    ...
    def build_initial_chat_history(
        ...
        """Initial messages: system (global) + system (identity-only for this role)."""
        return [
            {
                "role": "system",
    ...
    def get_role_description(definition: RoleDefinition) -> str:
        """Chinese description for RoleConfig / private notes."""
    ...
    def build_target_selection_prompt(
        ...
        *,
        ...
        """Chinese prompt for choosing one target ([[n]])."""
    ...
    def build_yes_no_prompt(
        ...
        """Chinese yes/no prompt ([[1]] yes, [[0]] no)."""
    ...
    def build_multi_target_prompt(
        ...
        """Chinese prompt for choosing multiple targets."""
        parts = [
    ...
    def parse_bracket_number(response: str) -> int | None:
        """Parse first integer from [[n]] or plain number."""
        import re
