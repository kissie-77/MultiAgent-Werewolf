...
def build_strategy_scores(
    ...
    """仅基于 strategy 维度上下文中的事件计分（默认使用全量 events 以兼容旧调用）。"""
    scores: dict[str, dict[str, Any]] = {
        pid: {"total": 0, "details": []} for pid in ctx.roster
    }

    for event in events if events is not None else ctx.events:
        ...
        if etype == "seer_checked":
            ...
            target_id = str(data.get("target_id", ""))
            if target_id_to_camp(target_id, ctx.roster) == Camp.WEREWOLF.value:
                scores[actor]["total"] += 25
                scores[actor]["details"].append("witch_poison_wolf")
            elif target_id_to_camp(target_id, ctx.roster) == Camp.VILLAGER.value:
