...
class RoutedMessage(BaseModel):
    ...
    private_thought: str | None = Field(
        default=None, description="Inner monologue visible only to the speaker"
    )
    ...
    audience_player_ids: list[str] = Field(
        default_factory=list, description="Resolved audience for CUSTOM or derived channels"
    )


def audience_for_channel(channel: VisibilityChannel, players: list[PlayerProtocol]) -> list[str]:
