...
def persist_run_artifacts(engine: Any, run_dir: Path) -> None:
    ...
    if state is not None and state.wolf_camp_minds:
        wolf_path = run_dir / "wolf_camp_mind.jsonl"
        if not wolf_path.is_file() and any(m.history for m in state.wolf_camp_minds.values()):
            from llm_werewolf.strategy.wolf.camp_mind import save_all_wolf_camp_histories

            save_all_wolf_camp_histories(state.wolf_camp_minds, wolf_path)
