...
class TestMsg:
    ...
    def test_get_text_content_with_separator(self):
        msg = Msg(
            name="Test",
            content=[
                {"type": "text", "text": "Hello"},
...
class TestMessageAdapter:
    ...
    def test_str_to_msg_default_role(self):
        msg = MessageAdapter.str_to_msg("测试消息")
        assert msg.role == "system"

    def test_str_to_msg_with_metadata(self):
        msg = MessageAdapter.str_to_msg(
            "测试消息",
            metadata={"round": 1, "phase": "day"},
        )
        assert msg.metadata["round"] == 1
        assert msg.metadata["phase"] == "day"

    def test_msg_to_str(self):
        msg = Msg(name="Test", content="测试消息", role="user")
        text = MessageAdapter.msg_to_str(msg)
        assert text == "测试消息"

    def test_msg_to_str_with_blocks(self):
        msg = Msg(
            name="Test",
            content=[
                {"type": "text", "text": "Hello"},
    ...
    def test_create_system_msg(self):
    ...
    def test_chat_history_to_msgs(self):
        history = [
    ...
    def test_msgs_to_chat_history(self):
        msgs = [
