...
from llm_werewolf.evaluation.correctness.scenarios import smoke_6p_basic
...
def test_runner_writes_artifacts_for_smoke_scenario(tmp_path: Path) -> None:
    ...
    assert (game_dir / "mvp_scores.json").is_file()
    assert (game_dir / "post_game_steps.json").is_file()
