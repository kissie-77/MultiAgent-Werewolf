...
def _load_script_module():
    script_path = (
        Path(__file__).resolve().parents[2] / "scripts" / "benchmark_vote_intention_parallelism.py"
    )
...
def test_intention_to_dict_serializes_dataclass_entries() -> None:
    ...
    entry = VoteIntentionEntry(
        player_id="p1", player_name="P1", seat=0, target_id=None, target_name=None, reason="test"
    )
